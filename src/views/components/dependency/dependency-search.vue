<template>
  <div class="rk-trace-search">
    <div>
      ...
      <div class="flex-h">
        <DependSelect :hasSearch="true" :title="this.$t('service')" :value="DependTrace.currentService" @input="selectService"
                     :data="DependTrace.services"/>
        <DependSelect :hasSearch="true" :title="this.$t('endpoint')"  :value="DependTrace.currentEndpoint" @input="selectEndpoint" :data="DependTrace.endpoints"/>
        <!--<InvokSelect :title="this.$t('status')" :value="traceState" @input="chooseStatus"-->
...
<script lang="ts">
    ...
    import DependSelect from './dependency-select.vue';

    @Component({components: {DependSelect}})
    export default class DependSearch extends Vue {
        ...
        @Action('DependTrace/SELECT_SERVICE') private SELECT_SERVICE: any;
        @Action('DependTrace/SELECT_ENDPOINT') private SELECT_ENDPOINT: any;
        @Action('DependTrace/GET_QUERY') private GET_QUERY!: any;
        @Action('DependTrace/MIXHANDLE_GET_OPTION') private MIXHANDLE_GET_OPTION: any;
        ...
        private selectService(i: any) {
            if (this.DependTrace.currentService.key === i.key) {
                return;
            }
        ...
        private selectEndpoint(i: any) {
            if (this.DependTrace.currentEndpoint.key === i.key) {
                return;
            }
...
//            this.SET_DEFAULT_EMPTY_TRACE();
            ...
            this.$eventBus.$emit('SET_DEPEND_LOADING_TRUE', () => {
                this.GET_DEPENDENCY_LIST({
                    ...
                    if(this.DependTrace.currentTrace.id) {
        ...
        private handleOption() {
            ...
            this.$eventBus.$emit('SET_DEPEND_LOADING_TRUE', () => {
                this.MIXHANDLE_GET_OPTION({duration: this.durationTime})
                    .then(() => {
                        this.$eventBus.$emit('SET_DEPEND_LOADING_FALSE');
                      if(this.DependTrace.currentEndpoint.key) {
                          this.handleSearch();
                      }
                    }).catch(() => {
                    this.$eventBus.$emit('SET_DEPEND_LOADING_FALSE');
