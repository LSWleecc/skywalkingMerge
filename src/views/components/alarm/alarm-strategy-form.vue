<template>
  <div style="width: 100%">
    <el-form :model="ruleForm" :rules="rule" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item :label="ruleForm.name == 'SLA' ? '阈值（%）': '阈值（ms）'" prop="region">
        <el-input-number :precision="ruleForm.name == 'SLA' ? 2 : 0" v-model="ruleForm.region" controls-position="right" @change="handleChange" :min="0" :max="ruleForm.name == 'SLA' ? 100 : Infinity">%</el-input-number>
      </el-form-item>
      ...
      <el-form-item style="margin-left: -100px">
        <el-col :span="6">
          <el-form-item label="周期" prop="intervalTime">
            <el-input-number v-model="ruleForm.intervalTime" controls-position="right" @change="handleChange" :min="1" :max="10"></el-input-number>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="次数" prop="count">
            <el-input-number v-model="ruleForm.count" controls-position="right" @change="handleChange" :min="1" :max="10"></el-input-number>
          </el-form-item>
        </el-col>
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item label="延迟期" prop="region">
        <el-input-number v-model="ruleForm.delayTime" controls-position="right" @change="handleChange" :min="1" :max="10"></el-input-number>
      </el-form-item>

      <el-form-item label="报警消息" prop="alarmMessage">
        <el-col :span="12">
          <el-input type="textarea" v-model="ruleForm.alarmMessage"></el-input>
        </el-col>
      </el-form-item>
      <el-form-item>
...
<script lang="ts">
  ...
  import { errEvent } from  '../../../utils/util';
  @Component({
    components: {
    }
  })
  export default class StrategyManage extends Vue {
    ...
    @Action('rocketAlarm/GET_METRICS_ALARM_RULE') private GET_METRICS_ALARM_RULE!: any;
    ...
    // @Action('rocketAlarm/GET_PERCENT_RESPONSE') private GET_PERCENT_RESPONSE!: any;
    ...
    public submitForm(ruleForm:any) {
      (this.$refs.ruleForm as any).validate(async (valid:boolean) => {
        if (valid) {
            if(this.type == 'add') {
                const check =  this.rocketAlarm.Strategylist.some((item) => {
    ...
    private mounted() {
      // this.GET_PERCENT_RESPONSE()
    }
    private created(){
        this.$eventBus.$on('EDIT_STRATEGY_FORM', this, (i:any) => {
          this.currentMetric = i;
          this.isDisable = true;
          this.type = 'edit';
          this.GET_METRIC_ALARM_RULE({id: i.id}).then((res) => {
        ...
        this.$eventBus.$on('ADD_METRIC', this, (i:any) => {
            this.currentNode = i;
            this.type = 'add';
